...
```lua
{
  ...
  keep_selection         = "never", -- Restore visual selection after commenting; "never"|"adjust"|"expand_block"|"expand_line"|"keep_visual"
...
Without `keep_visual`, `adjust` / `expand_block` / `expand_line` exit to normal
mode; use `gv` to re-enter the restored selection. `keep_visual` alone keeps the
original selection.
