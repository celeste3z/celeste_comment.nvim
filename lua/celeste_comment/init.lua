...
function H.make_operator(ctype, opts)
  ...
  local f = function(motion)
    ...
    if opts.visual and not state_track then
...
function M.setup(config)
  ...
  map("x", m.line_toggle_visual,  op_line_toggle_v,               { desc = "Line comment selection",     expr = true })
  ...
  map("x", m.block_toggle_visual, op_block_toggle_v,              { desc = "Block comment selection",    expr = true })
  map("n", m.line_invert,         op_invert,                      { desc = "Invert comment by motion",   expr = true })
  map("x", m.line_invert,         op_invert_v,                    { desc = "Invert comment selection",   expr = true })
  map("n", m.line_force_add,      op_force_add,                   { desc = "Force add line comment",     expr = true })
  map("x", m.line_force_add,      op_force_add_v,                 { desc = "Force add line comment",     expr = true })
  map("n", m.line_force_remove,   op_force_rmv,                   { desc = "Force remove line comment",  expr = true })
  map("x", m.line_force_remove,   op_force_rmv_v,                 { desc = "Force remove line comment",  expr = true })
  ...
  map({ "o", "x" }, m.auto_textobject, cmd("textobject_auto()"), { desc = "Auto line/block textobject" })
  map(
