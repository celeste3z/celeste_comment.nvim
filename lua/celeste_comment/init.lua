...
---@field cursor?  vim.Pos cursor position
---@field anchor?  vim.Pos visual anchor
---@field ncursor? vim.Pos new cursor
---@field nanchor? vim.Pos new anchor
---@field mode?    string 'v'|'V'|'\22'
...
do
  ...
  ---@overload fun(pos:vim.Pos?):vim.Pos?
  ---@overload fun(pos:vim.Pos):vim.Pos
...
---@return Celeste.Comment.Range4?
---@return Celeste.Comment.Motion?
function H.get_selection_range(buf, visual)
  if visual then
    ...
    local motion ---@type Celeste.Comment.Motion
    if mode == "v" then
      motion = "char"
    elseif mode == "V" then
      motion = "line"
    else
      assert(mode == "\22", "unexpected visualmode")
      motion = "block"
    end

    return { sr, sc, er, ec }, motion
  end
...
function H.make_state_track()
  ...
  if current_visual then
    ...
    state.anchor = H.make_pos(anchor[1], anchor[2] - 1, anchor[3] - 1)
  end

  state.ncursor = H.pos_clone(state.cursor)
  state.nanchor = H.pos_clone(state.anchor)
  return state
end

---@param v Celeste.Comment.StateTrack?
---@return string
function H.state_track_to_string(v)
  if v then
    return ("{mode:%s, cursor:%s, anchor:%s, ncursor:%s, nanchor:%s}"):format(
      v.mode,
      H.pos_to_string(v.cursor),
      H.pos_to_string(v.anchor),
      H.pos_to_string(v.ncursor),
      H.pos_to_string(v.nanchor)
    )
  end
  return "nil"
end
...
function H.expand_block(state, motion, edits, csi)
  ...
  if not state.cursor or not state.anchor then return end
...
function H.keep_selection_expand(state, ks, ctype, motion, edits, csi)
  ...
  if not state.ncursor or not state.nanchor then return end

  local mode = state.mode
  local range = { state.nanchor.row, state.nanchor.col, state.ncursor.row, state.ncursor.col }
...
function H.restore_state(ctx)
  ...
  if cfg.keep_cursor and state.ncursor then vim.api.nvim_win_set_cursor(0, H.pos_to_cursor(state.ncursor)) end
end
...
function H.compute_cursor_state(ctx)
  ...
  state.ncursor = H.compute_cursor_pos(state.ncursor, ctx)
  state.nanchor = H.compute_cursor_pos(state.nanchor, ctx)
end
...
function H.select_range(range, opts)
  ...
  vim._with({ noautocmd = true, keepjumps = true }, function()
    local visual = H.current_visual()
    if visual then
      vim.cmd.normal({ visual ~= mode and (mode .. "\27") or "\27", bang = true })
...
---@param cursor vim.Pos
---@param range  Celeste.Comment.Range4 range should be inclusive
---@param ctype  Celeste.Comment.CommentType
---@param action Celeste.Comment.Action
---@param motion Celeste.Comment.Motion
---@param opts?  Celeste.Comment.InvokeCtx
...
---@param ctype Celeste.Comment.CommentType
---@param opts Celeste.Comment.InvokeCtx?
function H.visual_action(ctype, opts)
  if H.is_disabled({ check_modifiable = true }) then return end
  opts = opts or {}
  local action = opts.action or M.ACTION.kToggle

  local state_track = H.make_state_track()
  local mode, cursor, anchor = state_track.mode, state_track.cursor, state_track.anchor
  assert(mode and cursor and anchor, "not in visual")

  vim.cmd.normal({ "\27", bang = true })

  local cms_ref_pos = H.pos_clone(cursor < anchor and cursor or anchor)

  local range, motion = H.get_selection_range(cursor.buf, true)
  if not range or not motion then return end
  ---@cast motion Celeste.Comment.Motion

  H.make_action_range(cms_ref_pos, range, ctype, action, motion, { cfg = opts.cfg, state_track = state_track })
end
...
function M.setup(config)
  ...
  vim.api.nvim_create_autocmd("ModeChanged", {
    ...
    callback = function(ev)
      ...
      if vim.startswith(ev.match, "no") then
        vim.schedule(function() H.state_track = nil end)
      else
  ...
  ---@param mode string|string[]|fun(lhs:string):(string|string[])
  ...
  local function map(mode, lhs, rhs, opts)
    ...
    if t == "string" and lhs ~= "" then
      if vim.is_callable(mode) then mode = mode(lhs) end
      --- @diagnostic disable-next-line: param-type-mismatch
  ...
  ---@param v string
  ---@return string
  local function cmd(v) return ('<cmd>lua require("celeste_comment").H.%s<cr>'):format(v) end

  ---@param ctype Celeste.Comment.CommentType
  ---@param action Celeste.Comment.Action
  ---@param opts? table<string, any>
  ---@return (fun(motion:Celeste.Comment.Motion):string)|string
  local function make(ctype, action, opts)
    opts = opts or {}
    if opts.visual then return cmd(("visual_action( %d, { action = %d } )"):format(ctype, action)) end
    return H.make_operator(ctype, { action = action, motion = opts.motion })
  end

  local C, A = M.CMT, M.ACTION

  -- stylua: ignore start
  local op_line_toggle      = make(C.kLine,  A.kToggle)
  local op_line_toggle_cur  = make(C.kLine,  A.kToggle,      { motion = "_" })
  local op_line_toggle_v    = make(C.kLine,  A.kToggle,      { visual = true })
  local op_block_toggle     = make(C.kBlock, A.kToggle)
  local op_block_toggle_cur = make(C.kBlock, A.kToggle,      { motion = "_" })
  local op_block_toggle_v   = make(C.kBlock, A.kToggle,      { visual = true })
  local op_invert           = make(C.kLine,  A.kInvert)
  local op_invert_v         = make(C.kLine,  A.kInvert,      { visual = true })
  local op_force_add        = make(C.kLine,  A.kForceAdd)
  local op_force_add_v      = make(C.kLine,  A.kForceAdd,    { visual = true })
  local op_force_rmv        = make(C.kLine,  A.kForceRemove)
  local op_force_rmv_v      = make(C.kLine,  A.kForceRemove, { visual = true })

  map("n", m.line_toggle,         op_line_toggle,                 { desc = "Line comment by motion",     expr = true })
  map("n", m.line_toggle_cur,     op_line_toggle_cur,             { desc = "Line comment current line",  expr = true })
  map("x", m.line_toggle_visual,  op_line_toggle_v,               { desc = "Line comment selection" })
  map("n", m.block_toggle,        op_block_toggle,                { desc = "Block comment by motion",    expr = true })
  map("n", m.block_toggle_cur,    op_block_toggle_cur,            { desc = "Block comment current line", expr = true })
  map("x", m.block_toggle_visual, op_block_toggle_v,              { desc = "Block comment selection" })
  map("n", m.line_invert,         op_invert,                      { desc = "Invert comment by motion",   expr = true })
  map("x", m.line_invert,         op_invert_v,                    { desc = "Invert comment selection" })
  map("n", m.line_force_add,      op_force_add,                   { desc = "Force add line comment",     expr = true })
  map("x", m.line_force_add,      op_force_add_v,                 { desc = "Force add line comment" })
  map("n", m.line_force_remove,   op_force_rmv,                   { desc = "Force remove line comment",  expr = true })
  map("x", m.line_force_remove,   op_force_rmv_v,                 { desc = "Force remove line comment" })
  map("n", m.line_add_below,      cmd('insert_comment("below")'), { desc = "Add comment below" })
  map("n", m.line_add_above,      cmd('insert_comment("above")'), { desc = "Add comment above" })
  map("n", m.line_add_eol,        cmd('insert_comment("eol")'),   { desc = "Add comment at end of line" })
  map("n", m.uncomment_auto,      cmd("uncomment_auto()"),        { desc = "Auto detect and uncomment" })
  map("o", m.auto_textobject,     cmd("textobject_auto()"),       { desc = "Auto line/block textobject" })
  map("x", m.auto_textobject,     cmd("textobject_auto()"),       { desc = "Auto line/block textobject" })
  -- stylua: ignore end

  map(
    function(lhs) return lhs == m.line_toggle_visual and "o" or { "o", "x" } end,
    m.line_textobject,
    cmd("textobject_linewise()"),
    { desc = "Linewise comment textobject" }
  )
  map(
    function(lhs) return lhs == m.block_toggle_visual and "o" or { "o", "x" } end,
    m.block_textobject,
    cmd("textobject_blockwise()"),
    { desc = "Block comment textobject" }
  )
  map(
    function(lhs) return lhs == m.line_toggle_visual and "o" or { "o", "x" } end,
    m.line_textobject_inner,
    cmd("textobject_linewise(true)"),
    { desc = "Linewise inner comment textobject" }
  )
  map(
    function(lhs) return lhs == m.block_toggle_visual and "o" or { "o", "x" } end,
    m.block_textobject_inner,
    cmd("textobject_blockwise(true)"),
    { desc = "Block inner comment textobject" }
  )
  map(
    { "o", "x" },
    m.auto_textobject_inner,
    cmd("textobject_auto(true)"),
  ...
  map("i", m.line_toggle_insert, function()
    local cursor = H.make_cursor(0)
    local range = { cursor.row, cursor.col, cursor.row, cursor.col }
    H.make_action_range(cursor, range, C.kLine, A.kToggle, "line", {
      insmode = true,
